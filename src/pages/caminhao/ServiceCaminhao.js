import React, { useEffect, useState } from 'react';
import { Image, Text, StyleSheet, View, TouchableOpacity, SafeAreaView, Alert } from 'react-native';
import { useNavigation } from '@react-navigation/native';
import { Alinhamento, Balanceamento, Revisao, TrocaOleo } from '../../assets/Service';

const uri = 'data:image/jpeg;base64,'

export default function CaminhaoCategory() {
  const navigation = useNavigation();

  // const [service, getService] = useState('');

  const alinhamentoService = () => {
    navigation.navigate('AlinhamentoCaminhao')
  }

  const balanceamentoService = () => {
    navigation.navigate('BalanceamentoCaminhao')
  }

  const revisaoService = () => {
    navigation.navigate('RevisaoCaminhao')
  }

  const trocaOleoService = () => {
    navigation.navigate('TrocaOleoCaminhao')
  }
  

  const HeaderService = () => {
    return (
      <View style={styles.header}>
        <Text style={styles.text}>SELECIONE A CATEGORIA</Text>
      </View>
    )
  }

  const Services = () => {
    return (
      <View style={styles.boxContainer}>
        <View style={styles.box}>
          <TouchableOpacity onPress={() => alinhamentoService()}>
            <Alinhamento />
          </TouchableOpacity>
        </View>

        <View style={styles.box}>
          <TouchableOpacity onPress={() => balanceamentoService()}>
            <Balanceamento />
          </TouchableOpacity>
        </View>

        <View style={styles.box}>
          <TouchableOpacity onPress={() => revisaoService()}>
            <Revisao />
          </TouchableOpacity>
        </View>

        <View style={styles.box}>
          <TouchableOpacity onPress={() => trocaOleoService()}>
            <TrocaOleo />
          </TouchableOpacity>
        </View>
      </View>
    )
  }

  return (
    <View style={styles.container}>
      <Image source={{ uri }} style={[styles.image, StyleSheet.absoluteFill]} />
      <SafeAreaView style={styles.container}>
        <HeaderService />
        <Services />
      </SafeAreaView>
    </View>
  )
}

const styles = StyleSheet.create({
  container: {
    flex: 1,
  },
  button: {
    width: 100,
    height: 40,
    borderRadius: 10,
    alignItems: 'center',
    justifyContent: 'center',
    marginVertical: 10,
    borderColor: '#000',
    borderWidth: 1,
  },
  text: {
    fontSize: 25,
    fontWeight: '400',
    color: 'black'
  },
  header: {
    width: '100%',
    height: '08%',
    alignItems: 'center',
    justifyContent: 'center',
  },
  boxContainer: {
    width: '100%',
    height: '85%',
    padding: 5,
    flexDirection: 'row',
    flexWrap: 'wrap',
  },
  box: {
    width: '50%',
    height: '50%',
    padding: 5,
  },
  inner: {
    flex: 1,
    backgroundColor: '#c8c8c8',
    alignItems: 'center',
    justifyContent: 'center',
    height: '100%'
  }
});